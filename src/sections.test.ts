...
describe("buildProfileSections: presence, order, slugs", () => {
  it("returns visible sections in canonical SDK order, excluding Links (sidebar)", () => {
    const profile = makeProfile({
      ...
      education: [
        { rkey: "e1", institution: "MIT", startedAt: "2010", endedAt: "2014" },
      ],
...
describe("buildProfileSections: structured detail", () => {
  it("renders a position with org, formatted date range, description, and skills", () => {
    ...
    const career = buildProfileSections(profile).find(
      (s) => s.id === "career"
    )!;
  ...
  it("renders a non-standard publication as a card: title links (DOI fallback), publisher, date", () => {
    ...
    const pubs = buildProfileSections(profile).find(
      (s) => s.id === "publications"
    )!;
  ...
  it("links a project by its URL and passes the description through Markdown", () => {
    ...
    const projects = buildProfileSections(profile).find(
      (s) => s.id === "projects"
    )!;
  ...
  it("renders an investment with company, amount, date, and Markdown description", () => {
    ...
    const investments = buildProfileSections(profile).find(
      (s) => s.id === "investments"
    )!;
  ...
  it("groups skills by category using the SDK labels", () => {
    ...
    const skills = buildProfileSections(profile).find(
      (s) => s.id === "skills"
    )!;
    ...
    const html = buildProfileSections(
      makeProfile({
        presentationDeliveries: [
          {
            rkey: "d1",
            title: "Spryker Partner Hackathon",
            eventName: "Spryker Partner Hackathon",
            date: "2023-07-08",
          },
        ] as never,
      })
...
describe("courses: linked credential is an actual link when the credential has a URL", () => {
  it("links the credential name to its credentialUrl", () => {
    const html = buildProfileSections(
      makeProfile({
        courses: [
          { rkey: "c1", name: "Leadership Program", credentialRkey: "cert1" },
        ] as never,
        certifications: [
          {
            rkey: "cert1",
            name: "Leadership Cert",
            issueDate: "2016-01",
            credentialUrl: "https://cred.example/x",
          } as never,
        ],
      })
...
describe("publications: long contributor lists collapse (#589)", () => {
  const many = (n: number) =>
    Array.from({ length: n }, (_, i) => ({ name: `Author ${i + 1}` }));
  const pubsHtml = (profile: Profile) =>
    buildProfileSections(profile).find((s) => s.id === "publications")!.html;

  it("lists every contributor when the list is short", () => {
    const html = pubsHtml(
      makeProfile({
        publications: [{ rkey: "p1", title: "T", contributors: many(5) }],
      })
    );
    expect(html).toContain("Author 1, Author 2, Author 3, Author 4, Author 5");
    expect(html).not.toContain("pub-o-contrib-all");
  });

  it("shows the first 10 names and a count, with the full list in a native disclosure", () => {
    const html = pubsHtml(
      makeProfile({
        publications: [{ rkey: "p1", title: "T", contributors: many(397) }],
      })
    );
    expect(html).toContain("Author 10, and 387 more");
    expect(html).toContain("Show all 397 authors");
    expect(html).toMatch(
      /<details class="pub-more pub-o-contrib-all"><summary>/
    );
    expect(html).toContain("Author 397");
  });

  it("keeps the profile owner visible after an ellipsis", () => {
    const contributors = [
      ...many(354),
      { name: "Teon L. Brooks", did: "did:plc:test", handle: "teonbrooks.com" },
      ...many(42),
    ];
    const html = pubsHtml(
      makeProfile({ publications: [{ rkey: "p1", title: "T", contributors }] })
    );
    expect(html).toMatch(
      /Author 10, \u2026, <a [^>]*>Teon L\. Brooks<\/a>, and 386 more/
    );
  });
});
